...
class StateMachine {
    ...
    async kmsRequest(request, options) {
        ...
        let abortListener;
<<<<<<< HEAD
        let kmsRequestTimeout;
        try {
            options?.signal?.throwIfAborted();
            if (this.options.kmsConnectCallback) {
                const remainingTimeMS = options?.timeoutContext?.csotEnabled()
                    ? options.timeoutContext.getRemainingTimeMSOrThrow(`KMS request timed out after ${options.timeoutContext.timeoutMS}ms`)
                    : undefined;
                ...
                const forwardAbort = (0, utils_1.addAbortListener)(options?.signal, function () {
                ...
                connectPromise.then(resolvedSocket => {
                    if (controller.signal.aborted)
                        resolvedSocket.destroy();
                }, () => null);
                const timeout = timeoutMS ? timeout_1.Timeout.expires(timeoutMS) : undefined;
                try {
                    ...
                    if (timeout_1.TimeoutError.is(err)) {
                        const timeoutError = new error_1.MongoOperationTimeoutError('KMS request timed out');
...
<<<<<<< HEAD
            const remainingTimeMS = options?.timeoutContext?.csotEnabled()
                ...
                : undefined;
            const timeoutMS = Number.isFinite(remainingTimeMS) ? remainingTimeMS : undefined;
            kmsRequestTimeout = timeoutMS ? timeout_1.Timeout.expires(timeoutMS) : undefined;
